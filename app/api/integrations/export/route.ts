...
import { validIntegrationAccessToken } from "@/lib/integrations/oauth";

const exportSchema = z.object({
  provider: z.enum(["hubspot", "onecrew", "webhook"]),
...
export async function POST(request: Request) {
  ...
  if (!ownerId) return json({ error: "Sign in before exporting an estimate." }, 401);
  ...
  if (payload.provider === "hubspot") {
    const token = process.env.HUBSPOT_PRIVATE_APP_TOKEN || await validIntegrationAccessToken(ownerId, "hubspot").catch(() => "");
    if (!token) return json({ error: "Connect HubSpot or add a private app token before exporting." }, 409);
  ...
  if (payload.provider === "onecrew") {
    const token = process.env.ONECREW_API_KEY?.trim();
    if (!token) return json({ error: "Add a OneCrew Lead Intake API key before exporting." }, 409);
    const response = await fetch(process.env.ONECREW_LEAD_INTAKE_URL || "https://app.getonecrew.com/api/v0/lead-intake", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        site_name: `Striping estimate — ${payload.address.slice(0, 120)}`,
        site_street_address: payload.address,
        project_lead_source: "Stripe Pros",
        project_note: `Estimate ready: $${payload.total.toFixed(2)} across ${payload.itemCount} priced scope items.`,
      }),
    });
    const result = await response.json().catch(() => ({})) as { message?: string };
    if (!response.ok) return json({ error: result.message || "OneCrew rejected the lead handoff." }, 502);
    return json({ ok: true, message: "Estimate sent to OneCrew Lead Intake." });
  }
